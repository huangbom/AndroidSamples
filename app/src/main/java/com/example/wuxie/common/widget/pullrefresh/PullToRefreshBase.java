...
import com.example.wuxie.common.widget.pullrefresh.ILoadingLayout.State;
import com.example.wuxie.common.widget.pullrefresh.loadlayout.FooterLoadingLayout;
import com.example.wuxie.common.widget.pullrefresh.loadlayout.HeaderLoadingLayout;
import com.example.wuxie.common.widget.pullrefresh.loadlayout.LoadingLayout;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T>
{
		...
		switch (ev.getAction())
		{
			...
			case MotionEvent.ACTION_UP:
				if (mIsHandledTouchEvent)
				{
					...
					{
						// 加载更多
						if (isPullLoadEnabled() && hasMoreData())
						{
							startLoading();
							handled = true;
						}
					...
					Log.d(TAG, "onTouchEvent: isReadyForPullDown:" + isReadyForPullDown());
					Log.d(TAG, "onTouchEvent: isReadyForPullUp():" + isReadyForPullUp());
				}
				break;
	...
	protected void resetHeaderLayout()
	{
		...
		if (refreshing)
		{
			mHeaderHeight = mHeaderHeight <=0 ? ScreenUtils.dp2Px(getContext(),98) : mHeaderHeight;
	...
	protected void startLoading()
	{
		...
		Log.d(TAG, "startLoading: "+mFooterLayout);
		if (null != mFooterLayout)
		{
	...
	final class SmoothScrollRunnable implements Runnable
	{
		...
		public void run()
		{
			...
			else
			{
				...
				final int deltaY =
					Math.round((mScrollFromY - mScrollToY) * mInterpolator.getInterpolation(normalizedTime / (float)oneSecond));
