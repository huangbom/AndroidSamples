...
public class NewbieGuideManager {
    ...
    public NewbieGuideManager addView(View target,View view,float xPercent,float yPercent) {
        mNewbieGuide.addRelativeView(target, view,xPercent,yPercent);
...
//                                .addIndicateImg(R.drawable.left_arrow, ScreenUtils.dpToPx(mActivity,
//                                60), ScreenUtils.dpToPx(mActivity, 110))
//                                .addMsgAndKnowTv("这个listview滚动到item6后出现新手引导浮层，\n只有点击我知道啦才会想消失", -ScreenUtils.dpToPx(mActivity, 250))
                                .show();
                        break;
                    case TYPE_COLLECT:
                        mNewbieGuide.addIndicateImg(android.R.drawable.arrow_up_float, ScreenUtils.dpToPx(mActivity, -70), ScreenUtils.dpToPx(mActivity,
                                50))
